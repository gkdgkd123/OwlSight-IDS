...
```bash
pip install -r realtime_ids/requirements.txt
```
...
export XGB_MODEL_PATH=./realtime_ids/models/xgb_model.json
...
python realtime_ids/main_realtime.py \
  ...
  --interface eth0 \
  --xgb-model ./realtime_ids/models/xgb_model.json \
...
```bash
python realtime_ids/main_realtime.py --interface eth0
```
...
```bash
# 测试 Suricata 监控模块
python -m realtime_ids.modules.suricata_monitor

# 测试早流特征提取模块
python -m realtime_ids.modules.early_flow_xgb

# 测试智能路由决策模块
python -m realtime_ids.modules.intelligent_router

# 测试 LLM 分析模块
python -m realtime_ids.modules.llm_analyzer
```
